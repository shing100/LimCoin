...
- 핸드셰이크는 한 왕복이라 남의 `HANDSHAKE` 를 그대로 베껴 보낼 수 있다.
  그러면 받는 쪽은 그 신원으로 받아들이지만, 베낀 쪽은 상대의 임시 개인키가
  없어 프레임을 하나도 만들거나 읽지 못한다 — 남는 것은 곧 끊기는 빈
  연결(방해)뿐이고, 가장하거나 엿듣지는 못한다.
...
| `GET /blocks/:hash` | |
| `GET /blocks/height/:h` | 높이로 한 개. 색인으로 바로 집는다 |
